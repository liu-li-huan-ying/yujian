...
/**
 * MathJax 渲染引擎 —— 「TeX → SVG / HTML」能力本身，**不含任何编辑器接线**。
 *
 * 放在 `src/render/` 而非 `src/editor/`：本模块被三方共用——
 *   1. 编辑器的行内 nodeView / 块级代码块预览（经 `src/editor/features/mathjax.ts` 接线）；
 *   2. 导出管线（`src/export/docTemplate.ts` 需要 renderLatexContent）；
 *   3. 公式临时编辑面板（`src/components/MathEditPanel.vue` 的实时预览）。
 * 其中导出层是「可在 Node 下直接测」的纯逻辑层，若它回头 import 编辑器层，
 * 就会被拖入 Milkdown / DOM 依赖而失去可测性。故渲染能力必须待在双方都够得着、
 * 且自身不依赖编辑器的那一层。
 *
 * 模块状态（标签编号表 / 待解析引用队列 / 令牌集合）均为**单实例**：
 * 凡需与它交互的代码必须经由本模块导入，不可另打包一份副本——否则
 * 「`\\label` 晚到 → 通知已定型 `(???)` 的引用重渲染」这条恢复链会断（见 EQREF-KNOWN-ISSUE）。
 */
...
export function onLabelsChanged(listener: LabelChangeListener): () => void {
...
export function hasRef(src: string): boolean {
 ...
 */
export function refUnresolved(svg: string): boolean {
