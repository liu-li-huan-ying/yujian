...
/** 选择参与 diff 的两侧文本：A↔B 对比 或 当前稿↔选中快照 */
const diffSides = computed<[string | null, string | null]>(() => {
  if (diffMode.value === 'ab') return [contentA.value, contentB.value]
  if (diffMode.value === 'selected') return [props.currentText, previewContent.value]
  return [null, null]
})
/** 把 diff 结果摊平成「逐行」列表，便于模板渲染（纯逻辑见 src/utils/snapshotDiff） */
const diffRows = computed(() => buildDiffRows(...diffSides.value))
const hasDiff = computed(() => hasChanges(diffRows.value))
const diffStats = computed(() => computeDiffStats(diffRows.value))

/**
 * 把扁平 diff 聚合成 hunks（pickText = B 侧可摘取内容）。
 * 原「行号累计」等易错逻辑已抽到 src/utils/snapshotDiff.buildHunks 并单测。
 */
const diffHunks = computed<DiffHunk[]>(() => buildHunks(diffRows.value))
