...
/* ── 快照对比 / diff（状态机在 composables/useSnapshotDiff）── */

/**
 * A/B 选点、快照内容读取、diffMode 与逐行 diff 在 `composables/useSnapshotDiff.ts`；
 * 渲染（变更段 / 增删统计 / 摘取高亮）在 `SnapshotDiffView.vue`。面板只做编排与绑定。
 */
const { compareA, compareB, toggleA, toggleB, diffMode, diffRows, diffSource, clearCompare } =
  useSnapshotDiff({
...
/** 统一 / 并排：状态留在面板 —— 面板根节点要靠它切 `.snap--split` 宽度 */
...
<template>
  <div class="snap glass" :class="{ 'snap--tl': view === 'timeline', 'snap--split': diffView === 'split' }" role="dialog" :aria-label="L.snapDialog">
    ...
    <!-- diff 预览（cherry-pick 单位 = 每个变更段 hunk）：渲染见 SnapshotDiffView.vue -->
    <SnapshotDiffView
      v-if="diffMode !== 'none'"
      v-model:view="diffView"
      :mode="diffMode"
      :rows="diffRows"
      :source="diffSource"
