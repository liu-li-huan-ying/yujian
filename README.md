...
* **全文搜索** — 基于 [MiniSearch](https://minisearch.search/) 倒排索引，搜索结果点击直接定位到命中行（自动切源码模式精确跳转）。
...
* **写作统计** — 状态栏实时显示「汉字数 · 英文词数 · 估算阅读时长」，点击唤起玻璃弹层查看汉字 / 词 / 字符（含 / 不含空白）/ 阅读时长细分 + 当前**选区统计** + SVG 进度环展示**写作目标**完成度（目标随会话持久化）。
* **凝神模式** — 标题栏「月亮 `moon`」图标一键进入。融合打字机与禅：当前光标行视口**垂直居中**（偏上 1/3、平滑滚动、失焦暂停），同时当前块**高亮**、其余块**淡化**（ProseMirror 非破坏性 Decoration，不污染文档），营造沉浸式写作氛围；状态随会话恢复。
...
| 9. Phase 2 | 多文档标签+查找替换+版本快照+写作统计+凝神(打字机/禅)模式+导出增强+写作辅助+断链检查 | 🔧 批次一已完成（多文档标签·文件内查找替换·选区字数）；批次二已完成（版本快照·写作统计·凝神模式）；批次三待排期（见 [`docs/PHASE2-PLAN.md`](./docs/PHASE2-PLAN.md)） |
...
**Phase 2 计划**：见 [`docs/PHASE2-PLAN.md`](./docs/PHASE2-PLAN.md)（多文档标签 / 查找替换 / 版本快照 / 写作统计 / 凝神·打字机·禅模式 / 导出增强 docx·ePub·LaTeX·PDF / 写作辅助包 / 断链健康检查）。**批次一（多文档标签·查找替换·选区字数）与批次二（版本快照·写作统计·凝神模式）已落地**，批次三待排期。
