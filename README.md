...
* **文件内查找 / 替换** — 已整合进左侧搜索框的「本文档」范围（不再有独立顶部按钮或 Ctrl+F 浮层）；与「全部」（vault 全文搜索）**共用同一套检索引擎**，仅查找范围限定在当前文档，命中点击即跳转到对应行；支持区分大小写 / 全词匹配、单文档替换与全部替换；**零冗余**（无独立查找实现）。命中在编辑器内以视图层高亮呈现（源码 CodeMirror / 所见即所得 ProseMirror 双模式对称），不进文档、不影响保真。
* **选区字数** — 状态栏实时显示当前选区字符数（中英混排同计）。

### 版本快照与写作辅助

* **版本快照** — vault 内 `.yujian-history/` 自动留档（独立于 `.mdeditor/` 缓存，建议进库 `.gitignore`）；标题栏「历史 `history`」图标唤起玻璃面板，可手动留档并加**备注**（如"发布前"）；列表按时间排序并显示字数差，选中任一份即以 `diff@7` 做**行级 diff 预览**（新增 / 删除 / 上下文三色），回滚 = 把快照内容载入编辑器并标脏、**不立即覆盖磁盘原文**（严守 Markdown 往返保真）。
...
* **链接健康检查** — 标题栏「更多 ⌄ · 链接健康检查」一键扫描整个 vault：识别失效的 `[[wikilink]]`（按基名 / 相对路径解析）、相对路径 Markdown 链接与图片引用（解析后判定目标文件是否存在），玻璃面板按 Wiki / 链接 / 图片三色分类列出断链（源文件 · 行号 · 目标 · **所在行原文预览**），支持类型筛选与按类型计数；**点击即跳转到该文档的断链具体行**（在当前编辑模式下滚动定位，不强制切源码），hover 可见原始链接与所在行；外部链接（http(s) / mailto / 协议相对 / `www.` 域名）与纯锚点自动跳过。纯解析、零依赖，并为 Phase 3 双链埋下解析能力。
* **导出增强** — 标题栏「下载 `download`」下拉：HTML / PDF / LaTeX 三选一，可叠加**自动目录**（PDF 恒开）/ **封面页**（取 frontmatter 标题·作者·日期）/ **图片内联**（自包含离线可读）/ **仅选中范围**（无选区回退整篇并提示）。**多文件合订**：同一下拉里的「多文件合订…」打开玻璃面板，按文件树顺序列出 vault 内全部 `.md`，勾选 + 上下移排序 + 合订标题 + 每篇另起页，HTML / PDF / LaTeX 任选——逐文件渲染并**按各自目录内联图片**后拼接，保证跨目录自包含。**导出前预览**：「导出前预览」开关（或合订面板内勾选）开启后，落盘 / 打印前先以玻璃浮层渲染真实排版（LaTeX 显示源码），确认再导出。PDF 走隐藏窗口打印管线，强制内联图片与 Mermaid 图表；LaTeX 用纯 TS 零依赖的 `markdownToLatex` 生成 `.tex`（公式 / 代码原样保留，转义不破坏 URL 与 `\`）。
...
* **导出 PDF** — 通过 `webContents.printToPDF` 输出，与编辑器内观感一致；带**自动目录**（取标题层级）、**A4 分页控制**（一级标题另起页、代码与表格不跨页断裂）与**可选封面页**（标题 / 作者 / 日期取自 frontmatter）。
* **导出 LaTeX** — 纯 TS 零依赖把 Markdown 转成可编译的 `.tex`，**不引入 pandoc**。公式与代码块**原样保留**：先把公式 / 代码 / 链接抽成占位符，再对正文做特殊字符转义并回填，避免 URL 里的 `_`、公式里的 `\` 被破坏；表格 / 列表 / 引用 / 图片 / 脚注转对应 LaTeX 环境，默认 `ctexart` 文档类支持中文（需 XeLaTeX）。
* **导出范围** — 整篇或**仅选中内容**（选中走选区序列化，源码模式取选区 Markdown 再渲染，两种模式产出同构；无选区回退整篇并提示）。
* **图片与图表** — 图片可**内联为 base64**（自包含、离线可读）或保留相对路径；**Mermaid 转内嵌 SVG**，PDF 与离线 HTML 不再依赖 CDN。PDF 强制内联——它经隐藏窗口加载临时文件，相对路径图片本就取不到。
...
| 9. Phase 2 | 多文档标签+查找替换+版本快照+写作统计+凝神(打字机/禅)模式+导出增强+写作辅助+断链检查 | ✅ 批次一已完成（多文档标签·文件内查找替换·选区字数）；批次二已完成（版本快照·写作统计·凝神模式）；批次三已落地（导出增强·写作辅助·断链检查，见 [`docs/PHASE2-PLAN.md`](./docs/PHASE2-PLAN.md)） |
...
**Phase 2 计划**：见 [`docs/PHASE2-PLAN.md`](./docs/PHASE2-PLAN.md)（多文档标签 / 查找替换 / 版本快照 / 写作统计 / 凝神·打字机·禅模式 / 导出增强 docx·ePub·LaTeX·PDF / 写作辅助包 / 断链健康检查）。**批次一、批次二、批次三均已落地**。
