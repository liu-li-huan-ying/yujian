...
* **中英 / 中数 ¼ em 自动间距** — 汉字与拉丁字母、数字之间自动补 ¼ em 视觉间隙（W3C《中文排版需求》§3.2.2 建议的引擎行为），与全角标点相邻不补；代码块内不生效、行内代码仍补。**源文件一字不改**。
* **中文强调禁斜体 + 标点挤压 + 避头尾** — 强调改用强调色（中文斜体公认难看）；连续全角标点收紧、行首行尾标点不再突兀；中文段落间距比西文多 4px，补偿全角标点的视觉密度。
* **纯渲染层实现** — 交给排版引擎的标准 CSS（`text-autospace` / `text-spacing-trim`）完成，零性能开销、不污染导出；偏好面板可逐项开关，默认全开。
* **双击按词选中** — 双击中文按**词边界**选中（此前会把一整串汉字连标点整段选走），双击「开源」即选中「开源」；英文与数字仍沿用编辑器原有选词逻辑。

### 大文档性能

* **10 万字长文不卡顿** — 对超长文档自动启用「分块渲染」：滚动时只渲染可视区域附近的段落，屏外内容交给浏览器按需渲染，长文翻滚与输入保持顺滑。
* **不牺牲编辑器能力** — 采用浏览器原生能力而非把内容移出 DOM，选区、输入法、查找、导出、双向链接全部照常工作；**源文件绝不改动**。
* **自适应开关** — 仅对超长文档生效，普通笔记零影响；滚动手感与阅读进度条稳定不跳变。
...
* **快捷键自定义（设置面板可改键位）** — 按 `F1` 或命令面板「快捷键」打开专用设置面板：逐条列出全部命令、右侧显示当前键位胶囊，点胶囊即录入下一个按键组合（`Esc` 取消、`Delete`/`Backspace` 解绑）；撞命令可一键抢占、撞系统保留键（`Ctrl+N/T/W`、打印、`F5/F11/F12`、开发者工具等）或编辑器保留键（`Ctrl+C/V/Z/B/I` 等）则拦下以防破坏基本操作。默认键位只有一份真相（命令目录 `commands.ts` 的 `keys` 字段），命令面板键帽与派发表同源读取，不再两处打架；只存覆盖、`''` 表示显式解绑，默认值升级会跟着走。
* **帮助面板（使用指南 + 关于）** — 标题栏「？」打开，玻璃浮层含「使用指南」与「关于」两页：使用指南用 10 段图文带你快速上手；关于页展示版本号与应用简介。**键位清单已迁至上述「快捷键」设置面板**（键位可自定义后，手抄键位表必然过期，故移除以保单一真相）。
* **命令面板（统一入口）** — 按 `Ctrl/Cmd+Shift+P` 唤起命令面板、`Ctrl/Cmd+K` 快速打开笔记：键盘即可执行几乎所有操作（打开笔记库、切换大纲 / 图谱、进入凝神、导出、切皮肤、打开快捷键设置……），面板增多后的最高效入口；活动栏按钮同样可触发。

### 个人知识管理（PKM）· v2.0.0 新增

* **命令面板** — 见上「标题栏与帮助 · 命令面板」；`Ctrl/Cmd+Shift+P` 检索并执行所有命令，`Ctrl/Cmd+K` 跳转任意笔记，面板增多后的统一入口。
* **双向链接 `[[wikilink]]`** — 在 Markdown 里用 `[[笔记名]]` 建立双向链接：编辑器内核为真节点，悬停工具条或标题栏「更多」可插入；右侧「反链」面板列出所有指向当前笔记的链接，点击即跳回来源。
* **标签 `#标签`** — 内联 `#标签` 标记 + frontmatter 双轨采集；左侧「标签」面板聚合全库标签，点击筛选。
* **内容地图 MOC** — 用 frontmatter 的 `moc` 标记把若干笔记聚成一张地图，左侧「内容地图」面板分组呈现，快速纵览主题脉络。
* **关系图谱** — 点击活动栏第 7 个按钮进入全屏 Canvas 图谱视图：节点为笔记、连线为链接，支持平移 / 缩放 / 拖拽、本地子图聚焦、按度数截断；并附无障碍列表视图（`prefers-reduced-motion` 下直接收敛到终态）。
* **重命名 / 移动自动改写 `[[引用]]`** — 重命名或移动文档（含整文件夹）时，所有指向它的 `[[链接]]` 自动更新，历史与附件一并迁移，零手动修链。
...
### ② F1 唤起「快捷键」设置 · 标题栏「？」唤起帮助

`F1` 直接唤起**快捷键设置面板**（已修复此前被按键守卫拦截的问题）：列出全部命令、可逐条改键，是键盘用户查阅与改键的入口。帮助（使用指南 + 关于）则移到标题栏「？」按钮，玻璃浮层双页、↑↓ 选择、Enter 执行、Esc 关闭、中英双语；原先附在帮助里的手抄键位表已移走（键位可配后必然过期，改由快捷键设置面板单一真相替代）。
...
# 端到端红线：真启动应用，断言「打开文档不写盘」（磁盘逐字节比对）
npm run e2e:open
...
| 10. Phase 3 | PKM（统一索引·数据安全·双链/反链·标签·内容地图 MOC·关系图谱）+ 命令面板 + 双栏 2×2 停靠布局 | ✅ 已完成（随 v2.0.0 发布，见 [`docs/PHASE3-PLAN.md`](./docs/PHASE3-PLAN.md)） |
...
**Phase 3 已随 v2.0.0 发布**：见 [`docs/PHASE3-PLAN.md`](./docs/PHASE3-PLAN.md)（统一索引层 · 数据安全 · 双链 / 反链 · 标签 · 内容地图 MOC · 关系图谱 · 命令面板 · 双栏 2×2 停靠布局）。
...
* **Windows**：产出 `release/yujian-<版本号>-setup.exe`（NSIS 安装包）——可自定义目录、默认创建桌面与开始菜单快捷方式「玉笺」。
