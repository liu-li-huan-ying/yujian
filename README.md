...
* **凝神模式 2.0「雾与纸」** — 标题栏「月亮 `moon`」一键进入：四幕进场（两侧退入雾中 → 框架收幕 → 纸卷微收 720→640 → 雾起锚定，~360ms），当前块青瓷微光、其余块按距离**五档雾化衰减**（前快后缓），光标行由「纸卷」缓动钉在锚点（粘贴大段匀速补偿、缓出刹住）。凝神中 `Esc` **轻退**——掀起 32px 玻璃信息栏（文件名 · 字数 · 保存时间 · 切换文档 · 退出），再按收起。设置面板可调锚点位置（偏上 1/3 / 黄金分割 / 正中）、雾化衰减（快中慢）、滚动平滑度、自动全屏，改即生效随会话持久化；全部动效尊重系统「减弱动效」偏好。
