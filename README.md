...
* **统一玻璃材质（所有浮层）** — 标题栏下拉、右键菜单、帮助 / 偏好 / 外观面板、编辑区 slash 菜单**共用同一套玻璃配方**，且随明暗模式自动切换（暗色墨玉半透、亮色羊脂玉半透），不再出现「标题栏下拉是暗玻璃、关于面板却是亮玻璃」的割裂。
...
| 7. 打磨    | 主题/皮肤、体积裁剪、快捷键提示、设置面板、标题栏重设计 | 🟡 进行中（皮肤 + 偏好/外观面板 + 标题栏图标工具栏 + 帮助/快捷键面板已完成；体积裁剪待做） |
| 8. 分发    | electron-builder 打包          | 🟡 配置就绪（已写 NSIS/dmg/AppImage 目标，待产出可分发安装包）           |

**已实现路线图之外的增强**：中英双语 i18n、切换工作文件夹、启动偏好设置、五套皮肤 + 明暗/跟随系统、应用图标（玉笺）、独立面板显隐、玉质滚动条、代码块自适应高度、标题栏图标工具栏重设计、帮助与快捷键面板（F1）、块操作手柄一致左轨重做、统一玻璃材质（所有浮层随明暗切换）、表格长串自动换行。

***

## 📄 许可证

[MIT](./LICENSE)。本项目仓库当前为私有仓库。
