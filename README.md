...
* **帮助面板** — 标题栏「？」或 `F1` 打开，含「快捷键」与「使用指南」双标签页：快捷键按文件 / 视图 / 命令 / 搜索 / 通用分组并以 `kbd` 键位胶囊呈现；使用指南用 10 段图文带你快速上手。
* **命令面板（统一入口）** — 按 `Ctrl/Cmd+Shift+P` 唤起命令面板、`Ctrl/Cmd+K` 快速打开笔记：键盘即可执行几乎所有操作（打开笔记库、切换大纲 / 图谱、进入凝神、导出、切皮肤……），面板增多后的最高效入口；活动栏按钮同样可触发。
