...
* 前端：`src/store/snapshots.ts`（Pinia，**只缓存当前文档的快照列表，不持有内容**；`setTags`；Phase B 增 `activeBranch` / `branches`（派生，主线恒排最前）/ `branchList`，`refresh` 兜底回落主线）；玻璃 `SnapshotPanel.vue`（浮于编辑区右上）：入口为**左缘活动栏「快照」按钮**，打开时经 `App.togglePanel('snapshot')` 触发 `snapshots.refresh` 拉取（与标签 / 内容地图 / 反链互斥）：视图切换、分支 chips、备注输入 + 保存、标签筛选 chips、A↔B 任意两点对比、标签 chips 内联增删、左侧时间 + 备注 + 字数差 `deltaChars`、选中→`snapshotRestore` 只读返回→行级 diff 预览、右下恢复/删除 + 右键 `ContextMenu`（restore/delete danger）、空态文案。恢复走 `EditorHost.loadMarkdownExternal`（灌入 + 标 dirty + 自动保存），**不立即覆盖磁盘原文**（守 §5.2 保真红线）。
...
* 玻璃浮层，入口：**左缘活动栏（`src/components/ActivityBar.vue`）「反链」按钮**（`Icon` 新增 `backlink` 三节点图标）。与标签 / 内容地图 / 快照**互斥**（`App.togglePanel` 统一收口），任一时刻只显一个浮层，避免多浮层叠压编辑区。
...
* 玻璃浮层，入口：**左缘活动栏「标签」按钮**（`Icon` 新增 `tag` 书签图标 + `chevron-right` 箭头）。与内容地图 / 反链 / 快照互斥，见 §反链面板条目说明。
...
* 玻璃浮层，入口：**左缘活动栏「内容地图(MOC)」按钮**（`Icon` 新增 `map` 图标）。与标签 / 反链 / 快照互斥，见 §反链面板条目说明。
