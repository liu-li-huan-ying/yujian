...
> 版本：v2.2.0 ｜ 日期：2026-09-15 ｜ 状态：已发布
> 本文所有依赖版本均经过 `npm view` 实测可获取，环境结论来自本机实际探测。
> 📚 文档全貌与「哪句话说了算」见 [`docs/README.md`](README.md) —— 本文件是**架构唯一事实源**，
> 审计/评审报告只对当时负责，落地后的实际行为一律以本文为准。
...
> 📌 本节只讲**当前机制**。该问题的完整取证归档（现象、**已被证伪的两版旧结论**、两次误修的事故回放、
> 接线铁律）在 [`docs/EQREF-KNOWN-ISSUE.md`](EQREF-KNOWN-ISSUE.md) —— 排查同类问题**先读它**，
> 免得照着旧结论白走一遍。
...
| **10. Phase 3** | PKM：批次零(缺陷+统一索引地基)→一(数据安全)→二(双链+反链)→三(标签+MOC+关系图谱)→四(中文排版+体验)→五(技术写作+发布) | ✅ 批次零已落地（统一索引层`vaultIndex.ts`+`minisearch`死依赖移除）；✅ 批次一已落地（完整性自检·整库备份恢复·外部修改冲突三选一·表格压测19/19）；✅ 批次二已落地（wikilink 真节点·点击跳转/一键创建·反链面板消费索引 `backLinks`；收尾三项：`[[` 自动补全浮层·未链接提及一键包裹·断链面板一键创建，见 §5.15）；✅ 批次三已落地（`#标签`·MOC·关系图谱，§5.16~5.25）；✅ 批次四已落地（中文排版·大文档分块·中文分词·快捷键自定义·结构治理，§5.26~5.36）；批次五（技术写作与发布闭环：BibTeX 引文·文档 lint·静态站导出·用户 CSS 片段）待启动 |
...
### 5.40.1 图谱「暗淡」的成因与提亮口径

成因不是配色不好看，而是**同色系灰对灰**：画布底是 `--hue-editor`（近黑），
而一跳节点 `--hue-text-2`、二跳及更远 `--hue-text-3`、连线 `t3 @ 35%`
——**除中心节点外没有任何一处用了强调色**，整屏只剩灰阶层次，灰底上看灰点自然「暗」。

提亮不是整体调亮，而是**重建层次**（改的是「谁该被看见」，不是「整体拉亮度」）：

| 元素 | 改前 | 改后 | 理由 |
| --- | --- | --- | --- |
| 常态连线 | `t3 @ 35%`，1px | `t3 @ 55%`，1.1px | 35% 在近黑底上近乎不可见，连线结构读不出来 |
| 高亮连线 | `accent @ 60%`，1.4px | `accent @ 85%`，1.8px | 与常态拉开更大反差，选中即「点亮」 |
| 局部二跳+ | `t3` 中性灰 | `accent @ 22%` 淡染 | 灰色在近黑底上没有「位置感」，淡染仍保留层次但不发灰 |
| 全局节点 | `t3` | 度 ≥3 → `accent @ 45%`，否则 `t2` | 让「枢纽笔记」自己浮出来，一眼看出结构重心 |
| 全局半径 | 一律 3.5 | 度 ≥4 → 4.5 | 同上，尺寸也参与表达重要性 |
| 中心节点 | 仅 accent 实底 + 1.5px 描边 | **加 2.6r 柔和光晕**（accent 35%→0 径向渐变）+ 描边 | 中心是阅读锚点，光晕让视线立刻落上去 |
| 所有节点 | 无描边 | `--hue-editor` 1px 描边切形 | 密集区里相邻节点会糊成一团，底色描边把每个点「切开」 |
| 标签 | 中心 / 选中 / 悬停 / 一跳 | 再增「度 ≥5 的枢纽」 | 重要节点不该因跳数远而没名字 |

⚠️ **光晕必须用径向渐变而非 shadowBlur**：`shadowBlur` 在每帧重绘的力导动画里是
性能杀手（尤其 300 节点），径向渐变是一次 `createRadialGradient` + 一次 fill，代价可忽略。
...
而手柄节点由 `BlockHandleView` 在 `create()` 时一次性建好、整个编辑器生命周期复用
（ProseMirror 的 plugin view，见 `block-edit/index.js` 第 1186 行 `view: () => new BlockHandleView(...)`），
故 `create()` 之后补一次属性即长期生效，**无需 MutationObserver**。
