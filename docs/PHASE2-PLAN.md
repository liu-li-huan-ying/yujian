...
| 全局/文件内查找替换  | ✅     | 复用统一 vault 索引层（vaultIndex.ts）；匹配计数/循环/选区范围/全局上下文片段+键盘导航+范围筛选 |
...
* **全局跨库**：复用统一 vault 索引层（vaultIndex.ts），结果面板展示「标题 + 上下文片段 + 行号」；可按「当前文件夹 / 全部 vault」筛选；搜索命中后可在左侧 `Sidebar.vue` 搜索框展开玉质「全局替换」区块（新增 `vault:replace` 通道），仅在命中文件范围内做字面量替换并写回磁盘（详见 §3.8）。
...
* **快照 git 化 Phase C（2026-08-31 落地）**：**段落级 cherry-pick**——diff 预览按「变更段」聚合成 hunk（相邻变更合并 + ±1 行上下文、边界不重叠），每段标「新增/删除/修改」色条并带「摘取」按钮；摘取把该段对比方（旧版/B 侧）内容插入当前文档光标处。所见即所得下用 Milkdown `parserCtx` 解析为节点再 `tr.insert`（标题/列表/代码块等语法正确渲染），失败兜底整篇重灌追加；纯删除段无可摘来源不显示按钮。`SnapshotPanel` 经 `@pick` 发文本，`App.vue.onSnapshotPick` 调 `host.insertText` + toast。设计文档原标「ROI 低暂缓」，按用户要求落地（见 `docs/SNAPSHOT-GIT-DESIGN.md`、ARCHITECTURE §5.10）。
...
* **架构**：需建 vault 级 link index（正向 + 反向），即统一 vault 索引层 `vaultIndex.ts`（批次零已建成）；图数据由 index 派生，不存原始图。
