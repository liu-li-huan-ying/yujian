...
* **单实例红线**：所见即所得走 `src/editor/zen.ts`（`$prose(() => createZenPlugin())` 注册），源码模式走 `SourceEditor` 的 `EditorView.scrollIntoView` 居中（同源 `isZenActive()` 开关）；两模式共用一个 `zenState.active` 模块级开关，`setZen` 切换后 dispatch 一个空事务触发 `apply` 重算装饰（空事务不改文档、不误标脏）。
...
* **批次二**：保存后 `.yujian-history/` 生成快照；手动留档可加备注；历史面板行级 diff 预览并回滚（仅改编辑器、不覆盖磁盘）；统计汉字/英文/字符/阅读时长+选中统计+目标进度环正确；凝神模式（打字机居中 + 当前块高亮/其余块淡化 + 失焦暂停）符合预期；`focusMode` 会话持久化恢复正确。
