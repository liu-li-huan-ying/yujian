...
> 状态：**Phase A + Phase B + Phase C 均已落地（2026-08-31）**——命名标签、任意两点对比、内容哈希去重、index.json 元数据层、向后兼容迁移（Phase A），线性时间轴/血缘视图、轻量草稿分支（Phase B），以及段落级 cherry-pick（Phase C）均已完成（见 §5）。
...
### Phase C — 段落级 cherry-pick（已落地，2026-08-31）
7. **段落级 cherry-pick**：diff 预览按「变更段」聚合成 hunk，每段标注「新增/删除/修改」并提供「摘取」按钮，把该段在**对比方（旧版/B）侧**的内容插入当前文档光标处。
   - **UI 打磨（参考融合）**：hunk 头标 kind 图标 + GitHub 风格 `@@ -o,s +n,s @@` 行号；diff 头标注**摘取来源**并提供**统一/并排**切换（并排 = GitHub split，左旧右新、del+add 配对）；内联着色取 Google Docs 风格（绿/红/灰 + `+`/`−`/`·` 行号槽）；摘取后整块短暂高亮 + 「已摘取」微态。视觉语言与面板其它部分统一（玉质/玻璃、青瓷强调色、`--hue-*` token）。
   - **插入保真**：所见即所得下用 Milkdown `parserCtx` 把摘取文本解析成 ProseMirror 节点再 `tr.insert`（标题/列表/代码块等语法正确渲染，而非字面量）；解析失败兜底整篇重灌追加，确保不丢字。
   - **范围语义**：在「快照 vs 当前」对比中摘取的是旧版内容；在「A↔B」对比中摘取的是 B 侧内容。纯删除段（仅当前稿有、对比方无）无可摘取来源，不显示摘取按钮。
   - revert（整段撤销回旧版）即「恢复」能力已覆盖，故 Phase C 只做 cherry-pick 这一更细粒度动作。

---

## 六、开放问题（待你拍板）

> Phase A + Phase B + Phase C 已于 2026-08-31 落地。以下为后续可打磨项，不影响已交付能力。
...
- **cherry-pick 落点提示**：当前摘取插入到「编辑器光标处」，若光标失焦则落到上次光标位置；后续可考虑「插入到文末」作为显式备选。
