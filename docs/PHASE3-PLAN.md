...
> 状态：**批次零已落地，批次一已完成（2026-09-01），批次二已落地（2026-09-02），批次三三块全部落地（#标签 + 标签聚合面板 2026-09-03 · 内容地图 MOC 2026-09-03 · 关系图谱 2026-09-11）**。基线 v1.1.0（2026-09-01 发布，三平台 CI 打通），Phase 2 三批次全落地。**批次四进行中**（命令面板 2026-09-11 落地；中文排版渲染层 2026-09-12 落地；大文档分块渲染 2026-09-12 落地；中文分词 · 双击选词 2026-09-12 落地），批次五待启动。
...
| 块                                 | 是否纳入  | 备注                                            |
| --------------------------------- | ----- | --------------------------------------------- |
| P0：`\eqref` 重载失效修复                | ⚠️ 待定论 | 纯逻辑层已排除时序竞态、三个连环坑已修；用户所报重载 `???` 需 Electron 抓 DOM 定论（见 `.workbuddy/memory/EQREF-KNOWN-ISSUE.md`） |
...
### 批次零：P0 缺陷修复 + 统一索引地基（最先做） ✅ 2026-09-01 已完成（除 `\eqref` 端到端待定论）
...
> ⚠️ 未解遗留：块级 `\eqref` 重载显示 `???` —— 单一定义见 `.workbuddy/memory/EQREF-KNOWN-ISSUE.md`。
> 三个连环坑（`tags:'ams'` / `data-c` 检测 / 编号漂移）与 token 失效挂起均已修复；**端到端仍需 Electron 抓 DOM 定论**。
> 旧文档的「重载时序竞态」假设已被纯逻辑探针证伪（`docs/REVIEW-OPTIONAL-2026-09-11.md` §4.2），**勿再据此推测动手**。

* **块级 `\eqref` 重载后显示 `???`**：~~当前「逐节点 pending/flush 异步竞态」在重载路径不可靠~~ —— 该假设已被纯逻辑探针**证伪**（`docs/REVIEW-OPTIONAL-2026-09-11.md` §4.2：任何合理重载时序均结算为 `(1)`）。悬置原因改定位在 **app 层重载时块级 `renderPreview` 是否被派发**，取证清单见 `.workbuddy/memory/EQREF-KNOWN-ISSUE.md` §5。~~文档级确定性两遍渲染~~ 仅在取证确认属「引用先于定义渲染」时才作备选方案，勿先动代码。
  * ⚠️ **实施前必须先在 Electron 实跑抓 DOM 验证**：确认重载后行内 `\eqref` 是否走 `MathInlineView`、block 的 label 是否在 inline 解析前已登记、`renderPreview` 是否被调用。**前两次修复均因凭推测改错层而被推翻**，勿重蹈。
...
* `src/editor/features/wikilink.ts`：真节点 `wiki_link`（$nodeSchema + $remark 改写正文 `[[...]]` → mdast 节点 + $inputRule 敲 `]]` 即转节点）+ to-markdown handler 原样输出 `[[target]]` / `[[target|alias]]` / `[[target#anchor]]` / `[[target#anchor|alias]]`，**往返保真**（红线 4/6；`npm test` D 段守护锚点不被丢）。
...
* **重命名 / 移动自动更新 `[[引用]]`**（2026-09-10 决策并落地；见下方「已决策」段与 `docs/ARCHITECTURE.md` 索引层章节）。
...
> ✅ **已决策并落地（2026-09-10）：重命名 / 移动笔记时自动更新全库 `[[引用]]`。**
> * 实现：`vaultIndex.rewriteLinksForMoves`（纯逻辑 + fs 编排）+ `vault.rewriteLinksThenRefreshIndex`（收口）。
> * 边界：**只改写「能解析到被移动文档」的链接**，断链一律不动；沿用原链接形态（基名式 → 新基名、路径式 → 新相对路径），**保留 `#锚点` 与 `|别名`**，其余正文逐字节不变（含 CRLF）。
> * 透明：返回被改写的文件 / 链接计数，渲染层提示「已同步更新 N 处引用」（中英双语）。
> * 顺序铁律：必须在「磁盘已迁移、索引仍是旧路径」的窗口内改写；随后 `refreshIndexAfterMove` 只动受影响条目，**不触发全库 reconcile**。
> * 目录移动：展开为「逐篇文档」移动；来源文件自身也在移动之列时按新路径读写。
> * 覆盖：`scripts/test-core.mjs` A / C / C2 / C3 段（含目录移动、来源自迁移、零命中不写盘、幂等）。
> * 另：`[[路径|显示名]]` 别名与 `[[目标#锚点]]` 锚点均已在批次二落地并纳入往返测试。

### 批次三：标签 + MOC + 关系图谱 ✅ 2026-09-11 已完成

> **进度（2026-09-11）**：`#标签` 内联真节点 + frontmatter 双轨采集 + 标签聚合面板（浏览/钻取）已落地（见 `docs/ARCHITECTURE.md` §5.16）；内容地图 MOC（frontmatter `moc: true` + 三组聚合面板）已落地（见 §5.17）；**关系图谱已落地（见 §5.25）**，批次三收官。
...
* `electron/main/vaultIndex.ts` 新增纯函数 `buildGraph(index, { center, maxHops, maxNodes })`：图数据由索引派生（边 = 已解析为绝对路径的 `outLinks`），本地子图沿「出链 ∪ 反链」的**无向邻接** BFS，全局超 300 节点按**度降序**确定性截断。`test-core` `[L]` 段 16 条断言守护；其中「互链去重」已证伪——摘掉去重守卫该断言立刻 FAIL。
* IPC `VAULT_GRAPH` + preload `getGraph`；主进程走 `getLiveIndex`，与其它索引消费通道同源。
* `src/components/GraphView.vue`：Canvas 2D + `d3-force` —— 该包**仓库内已有**（mermaid 的传递依赖），纯 JS、无 node-gyp，提升为直接依赖即可安全复用，**未新增任何编译型依赖**；平移 / 缩放 / 拖拽在 Canvas 上自行实现，故不需要 `d3-zoom` / `d3-drag`。
* 入口：左缘活动栏第 7 键「图谱」。图谱是**独立全屏视图**，激活时占用整个内容区、隐藏左右停靠栏，`EditorHost` 用 `v-show` 保留实例（不丢编辑状态）。含等价**列表视图**（Canvas 不可读 → 无障碍）与 `prefers-reduced-motion` 直接跳到收敛终态。

### 批次四：中文排版与写作体验（差异化护城河）

> **进度（2026-09-12）**：命令面板已落地（2026-09-11，见 `ARCHITECTURE.md` §5.26）；**中文排版（渲染层）已落地**（见 §5.27）——中英/中数 ¼ em 间距、中文强调禁斜体改色、标点挤压 + 避头尾、中文段落间距，纯 CSS（`text-autospace` / `text-spacing-trim`，Chromium 140+ 原生）+ 偏好面板开关，源文件零改写；**大文档分块渲染已落地**（见 §5.28）——Chromium 原生 `content-visibility: auto`（非 DOM 窗口化，保 ProseMirror 选区/装饰前提），阈值 `doc.content.size > 20000` 时对白名单块生效，并要求附带修掉 `focusBlock` 全量扫描与 `emoji` 装饰重复扫描；**中文分词 · 双击选词已落地**（见 §5.30）——浏览器原生 `Intl.Segmenter` + 保守双字回退，只在 CJK 上接管双击、只在单个文本节点内选词；**快捷键自定义已落地**（见 §5.31）——默认键位只在 `commands.ts`（HelpPanel 手抄的那份键位表已删），`keymap.ts` 归一/匹配 + `shortcuts.ts` 覆盖与冲突三态 + `App.vue` 查表派发，`F1` 进「快捷键设置」。**中文搜索质量已落地**（见 §5.32.1）——Unicode 词边界替换原生 `\b`（后者只认 ASCII、对中文是空操作），库级搜索与编辑器内搜索收敛到 `src/utils/regex.ts` 单一来源；**复杂元素临时编辑界面 · 公式已落地**（见 §5.32.2）——共用玻璃外壳 `ElementEditPopover.vue` + `MathEditPanel.vue`（MathJax 实时预览 + 4 组符号工具条 + `\label` 状态），以**替换** Crepe 那个无预览的行内公式文本框的方式接入。**表格列宽拖拽决定不做**：Markdown 表格语法不承载列宽，持久化与「往返保真」红线冲突（详见 §5.32.2）。
...
| 4 | 自动改写引用误改用户文件             | 只改「能解析到被移动文档」的链接、断链一律不动、保留锚点/别名、其余正文逐字节保真；返回条数并提示；`npm test` C 段回归守护           |
...
| 12 | 大文档虚拟滚动与 Crepe 选区/装饰冲突   | **已处置（2026-09-12）**：不做 DOM 窗口化（会破 ProseMirror DOM↔doc 映射），改用 Chromium 原生 `content-visibility: auto`——DOM 保持完整，选区/装饰/查找/导出一律不动；白名单排除自带绝对定位弹层的组件块（见 `ARCHITECTURE.md` §5.28） |
...
- **关联数据随文档迁移/清理（核心修正：原实现只搬了 .md）**：
  ...
  - 统一索引层：文件精确「移除旧路径 + 登记新路径」；目录展开为逐篇文档处理；并在**同一窗口内**先改写全库 `[[引用]]` 再刷新索引（顺序不可换，见 `` `rewriteLinksThenRefreshIndex` ``），避免依赖 watcher 的 1s 延迟窗口读到陈旧路径。
...
* **批次零**：⚠️ **重载含 `\eqref` 的文档稳定显示 `(1)`（非 `???`）—— 端到端尚待 Electron 抓 DOM 验证**（纯逻辑层已过：`verify:md` 覆盖引用先于定义、超时兜底、token 失效不挂起）；手动重输仍正常；图片加载无回归；`minisearch` 已移除且文档表述修正；索引层建成——5000 文件库下输入无感知退化、索引内存 < 100MB、搜索不再受 80 文件上限限制；删掉索引后静默重建无报错。
* **批次一**：自检能报出索引/磁盘不一致、孤儿快照、缺失附件并可一键修复；整库备份与恢复往返无损；外部修改冲突时提供三选一而非静默覆盖；表格增删行列/撤销压测无损坏。
* **批次二**：正文 `[[笔记名]]` 渲染为可点击链接；`[[` 触发补全；点击跳转目标笔记，不存在可一键创建；**保存后原文仍为 `[[笔记名]]`，且 `#锚点` / `|别名` 一个不丢（往返保真，`npm test` D 段守护）**；反链面板正确列出来源笔记与上下文；未链接提及可一键包裹成双链；新增 / 删除 / 重命名笔记后反链随之更新；**重命名 / 移动后全库指向该笔记的 `[[引用]]` 自动改写为新路径，断链不动，且提示已更新条数**。
...
1. **批次零已完成**（缺陷 + 死依赖 + 索引地基）；余项 `\eqref` 端到端待定论——**先 Electron 抓 DOM 取证，勿凭「重载竞态」推测动手**（该假设已证伪）。
2. 「重命名是否自动更新 `[[引用]]`」**已决策为自动更新并落地**（2026-09-10）；后续改动该链路须跑 `npm test`。
3. **UI 先行**：每批动手前先读 `docs/PHASE3-UI-DESIGN.md` 对应章节，按既定规格实现（令牌、材质层、组件状态、无障碍），避免各自发明界面。
4. 每批遵循「**先更文档再提交**」纪律；批次完成走 `npm run check`（typecheck + lint + test）门禁，触及 Markdown 往返 / 编辑器语法的改动另跑 `npm run verify:md`；涉及索引/性能的批次**必须用大样本库实测**，不能只看类型检查。
