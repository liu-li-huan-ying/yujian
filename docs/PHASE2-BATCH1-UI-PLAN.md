...
* **滚轮浏览时不重定位窗口**：`compute` 检测到 `browsing` 标志（用户正在滚轮浏览）时，把窗口起点锁定在动画位置、仅做贴边修正，不把窗口拽回 active；点选任意标签（`onTabClick` / 更多菜单选中）才把 `browsing` 复位、窗口重新跟随 active。这是「滚轮画廊能真正滑动」的关键——旧实现会在 active 落在窗口内时强制居中回 active，导致滚轮刚移走就被拽回，表现为「滚了像没滚」。
...
* **滚轮画廊式横滑（带阻尼 / 惯性）**：标签条 `wheel` 在**仅溢出时**接管（无溢出原样放行、绝不劫持页面滚动）。不再用「每次 ±1 瞬移」——而是把 `delta` 累积为**速度**，逐帧 `animStart += velocity`、**摩擦衰减 `velocity *= 0.85`** 做惯性滑行；单帧速度增量有上限、总速度**限速 ±0.5 标签/帧**，约每次滚轮滑 1 个标签并惯性收尾，**不会一下飞太远**。`browsing` 期间窗口不拽回 active；点选标签才复位。键盘党走「更多」菜单点选，无障碍不丢。

##### 容量精确测量（修复「空着位置却只显示 5 个 / 该显示 6 个却显示 5 个」）

`cap`（可见数）必须吃满可用宽度、且幂等。旧实现用「已知宽度平均值」估算未渲染标签、仅 2 轮收敛，估算偏保守 + 收敛不足 → 稳定少算 1 个。现改为：

* **挂载时一次性渲染全部标签测真实宽度**（`measuring` 临时把渲染列表扩成全量、下一帧 `scanWidths` 拿全部 `offsetWidth` 后再收口），`cap` 直接由真实宽度贪心得出（`packFrom(0, avail)`），根除估算少算。50 个 div 渲染无压力。
* **窗口实际渲染数再从窗口起点重贪心校准**（`packFrom(windowStart, avail)`）：窗口落在较宽标签区时仍保证末项不被裁切，同时 `cap`（全局容量）用于 `overflowCount` 与滑动范围。
* 首帧 `onMounted` 同步首算一次（挂载后 `clientWidth` 已可读），避免只渲染 1 个标签的闪烁；`document.fonts.ready` 后重算一次，避免缓存 fallback 字体下的宽度。
