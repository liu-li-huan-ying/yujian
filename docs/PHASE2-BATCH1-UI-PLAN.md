...
> ⚠️ **功能变更（2026-08-30）**：本计划中的「文档内查找浮层 `FindPanel.vue`」已按用户要求**移除**，仅保留左侧文件树全局/本文档搜索框；`Ctrl/Cmd+F` 现聚焦该搜索框。全局查找/替换经 `searchVault` / `replaceInVault` 实现，与「本文档」搜索共用同一套检索引擎（仅范围不同，零冗余）。
...
| 元素             | 材质                             | 理由（对应 `UI-DESIGN.md` §2.1 高度阶梯）                |
| -------------- | ------------------------------ | ----------------------------------------------- |
...
> **布局算法（采纳主人的修正：不固定可见数）**：先用「默认卡片宽度」估算可见区能放下几个 → `cap`；再让这 `cap` 个可见标签以 `flex:1` **均匀占满可见区**，永远不留空位。窗口缩放时 `cap` 自动增减、标签数跟着变、始终填满——从根上消灭「固定 N 个」带来的空位与抖动。

* **占满式布局**：`.tab` 用 `flex: 1 1 0; min-width: 0`，可见 `cap` 个标签等分剩余宽度；标题 `flex:1 + ellipsis` 自适应截断。可用宽度 = scroller `clientWidth`（徽标 / 更多按钮是其 flex 兄弟且 `flex-shrink:0`，已自动扣除，无需二次预留）。
* **`cap` 估算**：`cap = floor(avail / DEFAULT_TAB_W)`，并下探到 `avail/cap ≥ MIN_TAB_W`（`DEFAULT_TAB_W=168, MIN_TAB_W=96`），避免窗口极窄时卡片被压垮；`cap` 不超过总数。
* **以当前标签为中心的滑动窗口**：`compute` 以 `activeIndex` 为中心保留 `cap` 个可见位（`floor((cap-1)/2)` 前后各半），保证当前文档始终可见；其余按原顺序收进「更多」。切到任意标签都不会让它落在折叠区看不见。
* **`compute` 走 rAF 批处理**：切标签 / 缩放窗口导致的连续触发合并到一帧，避免同步回流抖动；首帧 `onMounted` 同步首算一次（挂载后 `clientWidth` 已可读，避免只渲染 1 个标签的闪烁），`document.fonts.ready` 后重算一次（避免缓存 fallback 字体宽度）。
* **滚轮浏览时不重定位窗口**：`compute` 检测到 `browsing` 标志（用户正在滚轮浏览）时，把窗口起点锁定在动画位置、仅做贴边修正，不把窗口拽回 active；点选任意标签（`onTabClick` / 更多菜单选中）才把 `browsing` 复位、窗口重新跟随 active。这是「滚轮画廊能真正滑动」的关键。
* **位置徽标「当前 / 总数」**：仅在发生折叠时出现（3 个标签不挂多余数字）；`11px --hue-text-3` + `tabular-nums`（数字等宽不抖）+ 左侧发丝分隔线；`.tabbar__pos` 设 `min-width: 76px` 钉死宽度，位数变化（`9/9` → `10/10`）不再挤压布局引起振荡。
* **「更多」按钮纯图标 + tooltip**：隐藏数交给位置徽标，避免两个数字打架。
* **「更多」菜单可滚动 + 高亮当前项**：`ContextMenu` 容器 `max-height + overflow-y`，30+ 项仍可滚动点全；`MenuItem` 加 `active` 标记，溢出列表里当前文档 accent 文字 + 半透底高亮。
* **渐隐提示边**：标签条左 / 右两侧各一道极细渐隐边（`.tabbar__fade--l/--r`，用 `--hue-base` 同源渐隐），当窗口左侧 / 右侧还有更多可滚内容时浮现（`showFadeL/showFadeR`），提示「这里还有」。
* **到头回弹反馈**：滚轮滚到左 / 右边界仍往同方向滚时，触发 `is-bounce-l/--r` 关键帧（`translateX ±5px` 弹回），给一点物理阻尼感，明确「到头了」。
* **滚轮画廊式横滑（带阻尼 / 惯性）**：标签条 `wheel` 在**仅溢出时**接管（无溢出原样放行、绝不劫持页面滚动）。把 `delta` 累积为**速度**，逐帧 `animStart += velocity`、**摩擦衰减 `velocity *= 0.85`** 做惯性滑行；单帧速度增量有上限、总速度**限速 ±0.5 标签/帧**，约每次滚轮滑 1 个标签并惯性收尾，**不会一下飞太远**。`browsing` 期间窗口不拽回 active；点选标签才复位。键盘党走「更多」菜单点选，无障碍不丢。
* **性能不退化**：编辑器单实例，tabs store 只存路径字符串；栏内 DOM 永远只渲染可见的 ~`cap` 个，50 / 100+ 标签时重活不随标签数增长。
